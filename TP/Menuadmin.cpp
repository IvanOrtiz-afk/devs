...
void Menuadmin::platosmejor_valor() ///Muestra siempre el top 3 de los platos mejor valorados de la semana
{
    ...
    std::vector <std::string> nombres_platos;
    ...
    for (int i=0; i<registros_menus; i++)
    {
        ...
        if (std::find(dias_semana_mes.begin(), dias_semana_mes.end(), menu_muestra.getfecha().getDia()) != dias_semana_mes.end())
        {
            ...
            {
                if (menu_muestra.getcant_valoracion()>0)
                {
                    ...
                    nombres_platos.push_back(menu_muestra.getnombremenu());
                    break;
                }
            }
        }
    ...
    int valor_aux2=0;
    int valor_aux3=0;
    std::string nombre_menu1 = "N/A";
    std::string nombre_menu2 = "N/A";
    std::string nombre_menu3 = "N/A";
    ...
    for (int unsigned x=0; x<valoraciones.size(); x++)
    {
        if (valoraciones[x]>valor_aux1)
        {
            ...
            nombre_menu3=nombre_menu2;
            valor_aux2=valor_aux1;
            nombre_menu2=nombre_menu1;
            valor_aux1=valoraciones[x];
            nombre_menu1=nombres_platos[x];
        }
        else if (valoraciones[x]>valor_aux2)
        {
            ...
            nombre_menu2=nombres_platos[x];
        }
        else if (valoraciones[x]>valor_aux3)
        {
            valor_aux3=valoraciones[x];
            nombre_menu3=nombres_platos[x];
        }
    }
    ...
    if(valor_aux1!=0)
    {
        system("cls");
        line('=', 60);
        std::cout << "TOP 3 DE MEJORES VALORACIONES DE ESTA SEMANA PARA " << esta_muestra.getnombreestablecimiento() << std::endl;
        line('=', 60);
    }
    if (valor_aux1!=0&&valor_aux2!=0)
    {
        if (valor_aux1 > 0)
        {
            std::cout << "1er puesto para: " << nombre_menu1 << " con una valoracion promedio de " << valor_aux1 << " puntos" << std::endl;
            line('-', 50);
        }
        if (valor_aux2 > 0)
        {
            std::cout << "2do puesto para: " << nombre_menu2 << " con una valoracion promedio de " << valor_aux2 << " puntos" << std::endl;
            line('-', 50);
        }
        if (valor_aux3 > 0)
        {
            std::cout << "3er puesto para: " << nombre_menu3 << " con una valoracion promedio de " << valor_aux3 << " puntos" << std::endl;
            line('-', 50);
        }
    }
    else if (valor_aux1!=0&&valor_aux2==0&&valor_aux3==0)
    {
        std::cout << "El unico plato valorado esta semana fue " << nombre_menu1 << " con una valoracion promedio de " << valor_aux1 << " puntos" << std::endl;
        line('-', 50);
    }
    system("pause");
}
...
void Menuadmin::platosmas_vendidos()
{
    ...
    for (int i=0; i<registros_consu; i++)
    {
        ...
        if (std::find(dias_semana_mes.begin(), dias_semana_mes.end(), consumo_muestra.getfecha().getDia()) != dias_semana_mes.end())
        {
            ...
            std::string nombre_delesta;

            int registros_comen=arch_comensal.CantidadRegistros(); // Aca arranco a buscar el establecimiento del comensal
            int registros_esta=arch_esta.CantidadRegistros();       //que hizo el consumo
            for (int k=0; k<registros_comen; k++)
            {
                comensal_muestra=arch_comensal.Leer(k);
                if (consumo_muestra.getcliente()==comensal_muestra.getIDcomensal())
                {
                    for (int h=0; h<registros_esta; h++)
                    {
                        esta_muestra=arch_esta.Leer(h);
                        if (comensal_muestra.getIDestablecimiento()==esta_muestra.getidestablecimiento())
                        {
                            nombre_delesta=(std::string)esta_muestra.getnombreestablecimiento();
                            break; //encontre el establecimiento y guardo su nombre
                        }
                    }
                }
            }

            for (int unsigned x=0; x<platos.size(); x++) //sumo cantidades de platos vendidos
            {
            ...
            if (!plato_encontrado)
            {
                platos.push_back(PlatoVendido(nombre_delplato, 1, nombre_delesta));
            }
        }
    }
    ...
    else if (hubo_consumos)
    {
        ...
        std::string nombre_esta1, nombre_esta2, nombre_esta3;
        ...
        for (int unsigned j=0; j<platos.size(); j++)
        {
            if (platos[j]._cantidadVendida>cant1)
            {
                cant3=cant2;
                nombre3=nombre2;
                nombre_esta3=nombre_esta2;
                cant2=cant1;
                nombre2=nombre1;
                nombre_esta2=nombre_esta1;
                cant1=platos[j]._cantidadVendida;
                nombre1=platos[j]._nombre;
                nombre_esta1=platos[j]._establecimiento;
            }
        ...
        if (cant1>0)
        {
            ...
            std::cout << "En el establecimiento " << nombre_esta1 << std::endl;
            line('-');
            if (cant2>0)
            {
                std::cout << "2do puesto" << std::endl;
                std::cout << nombre2 << " con una cantidad vendida de " << cant2 << " unidades" << std::endl;
                std::cout << "En el establecimiento " << nombre_esta2 << std::endl;
                line('-');
            }
            if (cant3>0)
            {
                ...
                std::cout << "En el establecimiento " << nombre_esta3 << std::endl;
                line('-');
            }
        }
        system("pause");
    ...
    do
    {
        ...
        for (int i=0; i<registros_consu; i++)
        {
            ...
            if (fecha_muestra==consumo_muestra.getfecha())
            {
                ...
                if (!plato_encontrado)
                {
                    ...
                    platos.push_back(PlatoVendido(nombre_aux, 1, nombre_delesta)); ///Comparar el string del struct con el const char*
                }
            }
        }
        if (!hubo_consumos)
        ...
        else if (hubo_consumos)
        {
            ...
            for (int unsigned j=0; j<platos.size(); j++)
            {
                ...
                std::cout << "Establecimiento " << platos[j]._establecimiento << std::endl;
                line('-');
            }
        }
        system("pause");
...
void Menuadmin::eliminarplato(Fecha fechafiltrar)
{
    ...
    do
    {
        ...
        for (int i=0; i<registros; i++)
        {
            ...
            if (id_esta==esta_muestra.getidestablecimiento())
            {
                ...
                {
                    std::cout << "[ADVERTENCIA] No se encontraron platos para este establecimiento en la fecha indicada." << std::endl;
                    system("pause");
        ...
        if (loop==true)
        {
            ...
            for (int i=0; i<tamanio; i++)
            {
                ...
                {
                    std::cout << "[ADVERTENCIA] Esta seguro que desea ELIMINAR el plato seleccionado?";
                ...
                {
                    std::cout << "Plato eliminado EXITOSAMENTE" << std::endl;
                    system("pause");
                    break;
                }
            }
...
void Menuadmin::listarplatos_semana()
{
    ...
    system("cls");
    line('*', 60);
    std::cout << "--- MENUES DE LA SEMANA PARA EL ESTABLECIMIENTO: " << esta_muestra.getnombreestablecimiento() << " ---" << std::endl;
    line('*', 60);
    ...
    do
    {
        ...
        case 5:
        {
            ...
            do
            {
                ...
                if (nuevoImporte < 0)
                {
                    std::cout << "[ERROR] No se puede establecer un importe negativo" << std::endl;
                    loop = false;
                    system("pause");
...
void Menuadmin::listar_valoraciones()
{
    Archivos <Menues> arch ("Menues.dat");
    Archivos <Establecimientos> arch2 ("Establecimientos.dat");
    Menues plato_muestra;
    ...
    if (!cancelar)
    {
        ...
        do
        {
            ...
            else if (hayPlatos==true)
            {
                ...
                registros=arch.CantidadRegistros();
                for (int x=0; x<registros; x++)
                {
                    plato_muestra=arch.Leer(x);
                    if (opcion==plato_muestra.getidmenu())
                    {
...
void Menuadmin::menuValoraciones()
{
    int opcion;
    bool loop=false;
    system("cls");
    std::cout << "--- VALORAR UN PLATO ---" << std::endl;
    line('-');
    std::cout << "1- VALORAR EL PLATO DE HOY" << std::endl;
    std::cout << "2- VALORAR UN PLATO POR FECHA " << std::endl;
    std::cout << "0- Volver " << std::endl;
    do
    {
        std::string entrada=entrada_valida("Pulse la opcion deseada: ", NUMERO_ENTERO);
        line('-');
        opcion=std::stoi(entrada);
        if (entrada=="0")
        {
...
void Menuadmin::cargarvaloracion()
{
    ...
    {
        system("cls");
        std::cout << "Ingrese el ID de su ESTABLECIMIENTO";
        std::string entrada=entrada_valida(", o pulse cero para volver", NUMERO_ENTERO);
        line('-');
        int id_esta=std::stoi(entrada);
        if (entrada=="0")
        ...
        int registros2=arch_esta.CantidadRegistros();
        for (int j=0; j<registros2; j++)
        {
            esta_muestra=arch_esta.Leer(j);
            if (id_esta==esta_muestra.getidestablecimiento())
        ...
        if (!esta_encontrado)
        {
            std::cout << "[ERROR] Establecimiento no encontrado o inexistente, intente nuevamente" << std::endl;
            system("pause");
        }
        if (esta_encontrado)
        {
            ...
            {
                plato_muestra=arch.Leer(i);
                if (fecha_hoy.hoy()==plato_muestra.getfecha()&&esta_muestra.getidestablecimiento()==plato_muestra.getesta()&&plato_encontrado==false)
                {
                    system("cls");
                    line('-', 70);
                    std::cout << "LISTADO DE MENUES PARA VALORAR PARA EL ESTABLECIMIENTO " << esta_muestra.getnombreestablecimiento() << std::endl;
                    line('-', 70);

                }
                if (fecha_hoy.hoy()==plato_muestra.getfecha()&&esta_muestra.getidestablecimiento()==plato_muestra.getesta())
                {
                    std::cout << std::endl;
                    plato_encontrado=true;
                    ...
                    line('-', 45);
                    std::cout << plato_muestra.toString() << std::endl;
                    if (plato_muestra.getcant_valoracion()==0)
                    {
                        std::cout << "El plato aun no ha tenido valoraciones" << std::endl;
                    }
                    else
                    {
...
void Menuadmin::listarfacturas()
{
    ...
    Archivos <Comensal> archComensales ("Comensales.dat");
    Factura fc_muestra;
    bool loop=true, comensal_encontrado=false;
    int seleccion_id, j=0;
    do
    {
        bool existeComensal = false;
        do
        {
            system("cls");
            ...
            int cantComensales = archComensales.CantidadRegistros();
            Comensal Comensal;

            for (int j = 0; j < cantComensales; j++)
            {
                Comensal = archComensales.Leer(j);
                if (Comensal.getIDcomensal() == seleccion_id)
                {
                    existeComensal = true;
                    break;
                }
            ...
            if (!existeComensal)
            {
                ...
                std::cout << "[ERROR] El ID #" << seleccion_id << " NO EXISTE" << std::endl;
                system("pause");

            }
        }
        while(!existeComensal);
...
void Menuadmin::verCC()
{
    ...
    do
    {
        ...
        for (int i=0; i<registros; i++)
        {
            ...
            {
                line('=', 70);
                std::cout << "Estado de cuenta del usuario " << std::string(cc_muestra.getnombrecomensal()) << " al dia de la fecha " << fecha_hoy.hoy().toString() << std::endl;
                line('=', 70);
        ...
        if (comensal_encontrado==false)
        {
            std::cout << "[ERROR] El usuario no tiene una Cuenta Corriente activa o no existe, intente nuevamente" << std::endl;
            loop=false;
            system("pause");
        }
    }
...
void Menuadmin::cargarestablecimiento() /// se agrego getline y cin.ignore para que nos deje ingresar cadenas con espacios.
{
    ...
    do
    {
        ...
        for(int i = 0; i < cantidad; i++)
        {
            Establecimientos aux = arch_establecimientos.Leer(i);
            if(aux.getidestablecimiento() >= idestablecimiento)
            {
                idestablecimiento = aux.getidestablecimiento() + 1;
            }
        }
...
void Menuadmin::modificarestablecimientos()
{
    ...
    {
        Archivos <Establecimientos> arch_establecimientosLectura ("Establecimientos.dat");
        ...
        do
        {
            ...
            int cantidad = arch_establecimientosLectura.CantidadRegistros();

            for (int i=0; i< cantidad; i++)
            {
                Establecimientos auxesta = arch_establecimientosLectura.Leer(i);
    ...
    if (hubocambios == true)
    {
        Archivos <Establecimientos> arch_escritura("Establecimientos.dat");

        if (arch_escritura.Guardar(estamodificado, pos))
        {
            std::cout << "Cambios guardados correctamente" << std::endl;
        }
        else
...

{
    ...
    do
    {
        ...
        for(int i = 0; i < cantidad; i++)
        {
            ...
            if(aux.getIDcomensal() >= idcomensal)
            {
                idcomensal = aux.getIDcomensal() + 1;
            }
        }
        if (idcomensal == 0)
        {
...
void Menuadmin::listarcomensales()
{
    ...
    bool loop=false, esta_encontrado=false;
    do
    {
        std::cout << "Ingrese ID del establecimiento" << std::endl;
        std::string entrada=entrada_valida(" o pulse cero para volver", NUMERO_ENTERO);
        line('-');
        if (entrada=="0")
        {
        ...
        int idestablecimiento=std::stoi(entrada);
        int registros_esta=arch_esta.CantidadRegistros();
        for (int j=0; j<registros_esta; j++)
        {
            esta_muestra=arch_esta.Leer(j);
            if (esta_muestra.getidestablecimiento()==idestablecimiento)
            {
                esta_encontrado=true;
                loop=true;
                break;
    ...
    if(loop==true)
    {
        ...
        for (int i= 0; i < cantidad; i++)
        {
            comensal_muestra = arch_comensales.Leer(i);
            if (comensal_muestra.getIDestablecimiento()==esta_muestra.getidestablecimiento())
            {
...
void Menuadmin::mostrarconsumos()
{
    Archivos <Comensal> arch_comensal ("Comensales.dat");
    Archivos <Consumos> arch_consumos ("Consumos.dat");
    Consumos consumos_muestra;
    Comensal comensal_muestra;
    bool loop=false, comensalencontrado=false, consumoencontrado=false;
    do
    {
        system("cls");
        std::cout << "Ingrese el ID del comensal" << std::endl;
        std::string entrada=entrada_valida("o pulse 0 para salir", NUMERO_ENTERO);
        int id_comensal = std::stoi(entrada);
        if (entrada=="0")
        {
            return;
        }
        ...
        for (int i=0; i<registros; i++)
        {
            ...
            if (id_comensal==comensal_muestra.getIDcomensal())
            {
                ...
                {
                    Consumos consumos_muestra = arch_consumos.Leer(j);
                    if (consumos_muestra.getcliente()==comensal_muestra.getIDcomensal()&&consumoencontrado==false)
                    {
                        system("cls");
                        line('=');
        ...
        system("pause");
        if(comensalencontrado==false)
        {
            std::cout << "[ERROR] Comensal NO encontrado o es incorrecto, intente nuevamente." << std::endl;
        }
    }
    while(loop==false);
}
...
void Menuadmin::listarpago()
{
    ...
    do
    {
        ...
        if (!encontro)
        {
            std::cout << "[ADVERTENCIA] No se encontraron pagos para este comensal o el mismo no existe" << std::endl;
...
void Menuadmin::modificarusuarios()
{
    ...
    if (hubocambios == true)
    {
        Archivos <usuario> arch_usuarios_escritura ("Usuario.dat");

        if (arch_usuarios_escritura.Guardar(usuariomodificado, pos))
        {
            std::cout << "Cambios guardados correctamente" << std::endl;
        }
        else
...
void Menuadmin::exportarDatosCSV()
{
    ...
    do
    {
        ...
        case 4:
        {
            ...
            salida.close();
            std::cout << "Se exportaron " << cant << " comensales." << std::endl;
            system("pause");
            break;
